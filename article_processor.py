...
def _load_cache():
    if os.path.exists(CACHE_PATH):
        try:
            ...
            logger.exception("Errore caricamento cache")
...
def generate_items(force=False):
    ...
    batch = links[cache.get("cursor", 0):cache.get("cursor", 0) + MAX_BATCH]
    for url in batch:
    ...
    cache["cursor"] = (cache.get("cursor", 0) + len(batch)) % len(links)
    _save_cache(cache)
    return len(batch)
