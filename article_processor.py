...
def save_cache(data):
    """Salva la cache su disco ordinata per data di pubblicazione e filtra link non validi."""
    ...
    for art in items:
        url = art.get("url", "")
        # esclude link mese, tag, favicon, immagini, ecc.
        if any(x in url for x in ["?month=", "/tag/", "favicon", ".ico", ".jpg", ".png"]):
            continue
        clean_items.append(art)

    # ordina per data (più vecchi prima)
    clean_items.sort(key=lambda x: x.get("published") or "")
    data["items"] = clean_items

    with open(CACHE_PATH, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)
    logger.info("✅ Cache ripulita e salvata di nuovo (%s articoli validi).", len(clean_items))
...
def generate_items():
    ...
    try:
        ...
        blog_links = [l for l in all_links if "/blog/" in l and "?month=" not in l and "/tag/" not in l and "favicon" not in l]
...
# 🧹 Esegui una pulizia forzata immediata appena viene lanciato (una tantum)
if __name__ == "__main__":
    cache = load_cache()
    save_cache(cache)
    print("✅ Cache ripulita e salvata di nuovo (esecuzione diretta).")
